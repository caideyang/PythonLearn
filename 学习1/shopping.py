...
# 定义一个loadDatabase函数，用于从json文件中读取信息，默认加载的数据库是user_info.json文件
...
# 定义一个函数setShoppingTime,设置并记录购物的时间，函数里面定义了一个参数account，用于保存账户信息
def setShoppingTime(account):
    database = loadDatabase() # 设定要记录到哪个数据库，这里使用的是前面定义好的函数loadDatabase定义的user_info.json
    #获取当前时间，并进行格式转换
    dt = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    database[account]['shopping_time'] = dt # 将转换好的时间记录到字典里面的shopping_time键上
    saveAccount(database) # 保存购物时间到数据库中，这里的数据库是指user_info.json文件

# 定义一个函数，用于获取已经保存过的购物时间
def getShoppingTime(account):
    database = loadDatabase()
    data = database[account]['shopping_time']
    # 返回变量data的值，变量data保存的就是account键对应的购物时间值，这个值是从json里面解码出来后到字典里
    # 由json到Python可识别的字典数据的解码过程由loadDatabase函数完成
    return data

# 定义一个函数getShoppingHistory，用于查询购物历史记录
def getShoppingHistory(account):
    ...
    for j in aa:
        ...
        date = getShoppingTime(account)
        # 打印购买的商品的数量、日期和商品名称
        print('您于%s购买了%s件%s' %(date,count,j))

# 定义一个函数login，用于登录系统
def login():
    database = loadDatabase() # 加载数据库，使用的是前面定义好的加载数据库的函数
    print(database)
    blacklist = loadDatabase('BlackList.json') # 设置用户的黑名单列表，在列表中的用户将不允许登录到购物系统
        ...
        # 这里使用while循环和count计数器，如果输入错误密码大于3次，则锁定账户
            ...
            while count < 3:
                ...
                if pwd == database[account]['pwd']:
                    # 进入到死循环
                    while True:
                        # 首先登录成功后，先获取用户账户的余额，告诉用户还剩多少钱，余额通过getAccountBalance函数得到
                        ...
                        if command == 'h':
                            ...
                            if database[account]['shopping_time'] != None:
                                # 加载函数getShoppingHistory，输出购物历史信息
                                getShoppingHistory(account)
                            else:
                                print('您在本商城没有购买过东西！')
                        elif command == 't':
                            # 如果用户输入的是t，则加载定义好的recharge函数，执行充值操作
                            recharge(account)
                        elif command == 's':
                            ...
                            # 调用saveAccount函数，将清空的操作保存到database文件
                            saveAccount(database)
                            # 调用shopping函数，开始购物
                            shopping(account)
                        else:
                            # 如果用户的操作不符合上面所有的情况，则输出错误信息
                            # 这里直接调用前面定义好的error_info函数，输出错误信息
                            print(error_info)
                else:
                    count += 1
                    # 告诉用户还有多少次机会尝试登陆
...
# 定义一个函数setShoppingList，用于设定购物清单
def setShoppingList(account,c_name,num):
    database = loadDatabase()
    # 使用for循环添加购买的商品的数量
    for i in range(num):
        # 将购买的商品添加到字典shopping_list键所对应的列表中
        database[account]['shopping_list'].append(c_name)
        # 将购买信息通过调用saveAccount函数保存到json文件中
        saveAccount(database)

# 定义一个函数setBalance，用于计算购买商品后，所剩下的余额
def setBalance(account,num):
    database = loadDatabase()
    # 购买商品后，扣除购买的商品的价格
    database[account]['balance'] -= num
    # 将余额信息通过调用saveAccount函数保存到json文件中
    saveAccount(database)
...
def recharge(account):
    ...
    while True:
        ...
        if num.isdigit():  # 判断输入是否为纯数字(充值的数据必须为纯数字)
            ...
            saveAccount(database)  # 保存到文件
            account_balance = getAccountBalance(account)  # 再从文件中读取余额信息
            # 高亮打印充值后的余额信息
...
# 定义一个函数getAccountBalance，用于获取账户余额
            ...
            while True:
                ...
                # 如果用户输入的名称在二级菜单里面，则通过for把用户选择的二级菜单商品下面的三级菜单遍历出来
                    ...
                    while True:
                        ...
                        if p_name == 'b':
                            break
                        # 如果用户输入的q，则直接调用logout函数，退出购物
                        elif p_name == 'q':
                            logout(account)
                        # 如果用户输入的h，则查询json文件中的购物历史
                        elif p_name == 'h':
                            database = loadDatabase()
                            # 有购物历史，则调用getShoppingHistory函数，输出购物历史
                            if database[account]['shopping_time'] != None:
                                getShoppingHistory()
                            # 梅花购物历史的话，则告诉用户无购物历史
                            else:
                                ...
                                if num.isdigit():
                                    ...
                                    # 如果用户的账户余额小于商品的金额，则告诉用户余额不足，无法购买
                                        ...
                                        if g == 'b':
                                            break
                                        if g == 't':
                                        # 这里我增加了一个调用recharge函数，账户余额不住的时候，马上提示用户充值
                                            recharge(account)
                                # 如果用户购买商品时输入的数量不是纯数字类型，则调用error_info函数，输出错误信息
                                else:
                                    print(error_info)
                        # 如果用户输入的购买的商品名称不正确，则调用error_info函数，输出错误信息
                        else:
                            print(error_info)
                # # 如果用户输入的购买的商品分类信息不正确，则调用error_info函数，输出错误信息
                else:
                    print('输入错误，请重新输入！')
        # 如果用户输入的不是商品分类，而是q，则调用logout函数退出登录
        elif choose == 'q':
            logout(account)
...
#
def addUser():
    # 首先调用loadDatabase函数，将json文件解码出来，放到database变量
    ...
    while True:
        ...
        # 如果输入的密码是正确的，则告诉用户创建账户成功
            ...
            # 调用saveAccount函数，将创建好的账户信息和账户的初始余额、初始购物时间信息保存到json文件
            saveAccount(database)
            # 退出while循环
            break
...
def main():
    while True:
        ...
        # 如果用户输入的是大写的A，则调用addUser函数，创建新用户
        if command.upper() == 'A':addUser()
        # 如果用户输入的是大写的B，则调用login函数，进行登录
        elif command.upper() == 'B':login()
        # 如果用户输入的是Q，则调用logout函数，退出登录
        elif command.upper() == 'Q':logout()
