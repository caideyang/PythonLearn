...
def staff_info(func_type):
    def backup(func):
        def re_name(*args,**kwargs):
            ...
            # 这种情况下，代表文件有改动过，就需要对改动前的employee.info文件先进行备份
            if func_type != 'search_type':
                backup_time = time.strftime('%Y%m%d_%H%M%S')
                shutil.copyfile('employee.info','staff_info_%s.bak' % backup_time)
                print("原始员工信息表已备份为 staff_info_%s.bak" % backup_time)
            print("已有员工信息如下：")
            # 备份完成之后，将当前员工信息表的内容打印出来
            with open('employee.info','r',encoding='utf-8') as f:
                for line in f:
            ...
            # 如果func_type传入的值是mod_type或者del_type，代表是修改或删除了文件，这个时候做下面的操作：
                # 1. 因为原始的employee.info文件已经备份过，所以这时候可以把原始文件删除掉了
                # 2. 将函数中新生成的staff_info_new文件改名为employee.info，代表最终删除或修改完成
            if func_type in ['mod_type','del_type'] and os.path.exists('staff_info_new'):
                os.remove('employee.info')
                os.rename('staff_info_new','employee.info')
