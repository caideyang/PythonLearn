...
class CertForDog:
    ...
    def __init__(self,excel,pic_path):
        ...
        self.font2 = {
            ...
            "parentsfont": ImageFont.truetype("C:\Windows\Fonts\simhei.ttf", 19),  # 父母
            "grandparentsfont": ImageFont.truetype("C:\Windows\Fonts\simhei.ttf", 18),  # 祖父母
            "ggrandparentsfont": ImageFont.truetype("C:\Windows\Fonts\simhei.ttf", 17),  # 曾祖父母
        }
        self.ggandpa  =  []
        self.ggpandma =  []
    def getExcel(self):
    ...
    def createCertDetail(self,template_path,dogs_info):
        for dog_info in dogs_info:
            ...
            if  dog_info[-1] == "new":
                ...
                draw.text((160, 80), dog_info[2].title(), (0, 0, 0), font=self.font2["namefont"]) #狗狗名称
                ...
                draw.text((160, 55), dog_info[2].title(), (0, 0, 0), font=self.font2["namefont"]) #狗狗名称
    ...
    def createCert(self, template_path, dogs_info):
        for dog_info in dogs_info:
            image_file = template_path + '\\' + dog_info[-1] + ".qqqqq      ``` ```1`11 `1`111111jpg"  # 获取模板文件
            img = Image.open(image_file)
            # 新建一个画板
            draw = ImageDraw.Draw(img)
            if dog_info[-1] == "new":

                draw.text((340, 330), dog_info[2].title(), (0, 0, 0), font=self.font["namefont"])  # 狗狗名称
                draw.text((340, 415), dog_info[3].title(), (0, 0, 0), font=self.font["breedfont"])  # 品种
                draw.text((340, 577), dog_info[4].title(), (0, 0, 0), font=self.font["colorfont"])  # 毛色
                draw.text((340, 498), dog_info[5].title(), (0, 0, 0), font=self.font["sexfont"])  # 公母
                draw.text((340, 666), dog_info[6].title(), (0, 0, 0), font=self.font["birthfont"])  # 出生日期
                draw.text((340, 738), dog_info[7].title(), (0, 0, 0), font=self.font["breaderfont"])  # 犬主人
                draw.text((340, 814), dog_info[8].title(), (0, 0, 0), font=self.font["breaderfont"])  # 繁殖者
                draw.text((340, 980), dog_info[9].title(), (0, 0, 0), font=self.font["codenumfont"])  # 芯片号码
                draw.text((340, 900), dog_info[10].title(), (0, 0, 0), font=self.font["codenumfont"])  # 犬住址
                draw.text((900, 513), dog_info[11].title(), (0, 0, 0), font=self.font["parentsfont"])  # 父亲信息
                draw.text((900, 875), dog_info[12].title(), (0, 0, 0), font=self.font["parentsfont"])  # 母亲信息
                draw.text((1191, 432), dog_info[13].title(), (0, 0, 0), font=self.font["grandparentsfont"])  # 祖父信息
                draw.text((1190, 608), dog_info[14].title(), (0, 0, 0), font=self.font["grandparentsfont"])  # 祖母信息
                draw.text((1191, 795), dog_info[15].title(), (0, 0, 0), font=self.font["grandparentsfont"])  # 外祖父信息
                draw.text((1190, 965), dog_info[16].title(), (0, 0, 0), font=self.font["grandparentsfont"])  # 外祖母信息
                draw.text((1507, 385), dog_info[17].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 曾祖父
                draw.text((1507, 480), dog_info[18].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 曾祖母
                draw.text((1507, 564), dog_info[19].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 增外祖父
                draw.text((1507, 660), dog_info[20].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 增外祖母
                draw.text((1507, 747), dog_info[21].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 外曾祖父
                draw.text((1507, 845), dog_info[22].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 外曾祖母
                draw.text((1507, 930), dog_info[23].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 外曾外祖父
                draw.text((1507, 1025), dog_info[24].title(), (0, 0, 0), font=self.font["ggrandparentsfont"])  # 外曾外祖母
                tmp_number = dog_info[6].replace('/', '') + dog_info[9][-2:]
                # cert_id = tmp_number[0] + tmp_number[2:]+"\nCKU-000390409"
                cert_id = tmp_number[0] + tmp_number[2:]
                draw.text((281, 143), cert_id, (0, 0, 0), font=self.font["grandparentsfont"])  # 证书编号
            else:
                draw.text((340, 385), dog_info[2].title(), (0, 0, 0), font=self.font["namefont"])  # 狗狗名称
                ...
                draw.text((340, 980), dog_info[9].title(), (0, 0, 0), font=self.font["codenumfont"])  # 芯片号码
                draw.text((895, 525), dog_info[11].title(), (0, 0, 0), font=self.font["parentsfont"])  # 父亲信息
                draw.text((895, 890), dog_info[12].title(), (0, 0, 0), font=self.font["parentsfont"])  # 母亲信息
...
if __name__ == "__main__":
    ...
    cert = CertForDog('D:\\mydogs\\20180527.xlsx','D:\\mydogs\\20180527')
    #创建导出位置
    cert.crateDir()
